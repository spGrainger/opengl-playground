...
namespace stl {
  ...
  std::vector<triangle> parse_stl(const std::string& stl_path) {
    ...
    for (unsigned int i = 0; i < num_triangles; i++) {
      auto normal = parse_point(stl_file); //ignore the normal, still read it
      auto v1 = parse_point(stl_file);
      auto v2 = parse_point(stl_file);
      auto v3 = parse_point(stl_file);

      triangles.push_back(triangle(normal, v1, v2, v3)); //add triangle to list
      char dummy[2];
      stl_file.read(dummy, 2); //ignore last 2 bytes
    }
